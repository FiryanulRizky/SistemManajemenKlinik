...
<html>
  <head>
    ...
    <title>SMKlinik | Edit Obat</title>
  </head>
  <body>
    <header>
      <div class="container">
      ...
      <div class="container">
        ...
        <div class="report">
					<?php
						...
						$sql = mysqli_query($connect,"SELECT * FROM tb_obat WHERE no_obat = '$no' ");
						$data = mysqli_fetch_array($sql);
					 ?>
          <form class="add" action="" method="post">
            <table>
              <tr><td><input type="text" name="no" value="<?php echo $data['no_obat'] ?>" hidden=hidden></td></tr>
              <tr>
                <td>No. Bahan</td>
                <td><input type="text" name="no" value="<?php echo $data['no_obat'] ?>" disabled=disabled></td>
              </tr>
              <tr>
                <td>Nama</td>
              ...
              <a href="data_obat-a.php"><button type="button" name="back">Kembali</button></a>
					...
					<?php
						if(isset($_POST['upbutton'])){
							...
							mysqli_query($connect,"UPDATE tb_obat SET nama = '$nama', deskripsi='$des', stok='$stok', harga='$harga' WHERE no_obat= '$no'");
